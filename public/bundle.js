...
var doesExist = function doesExist(data) {
...
var renderProduct = function renderProduct(data) {
  var productHTML = bindDataToHTML(data);

  var sku = '#' + data.sku;

  if ($(sku).length) {
    // if the SKU exists already, increment quantity else add it to the table
    var $productQty = parseInt($(sku + ' .quantity').text());
...
var handleDecrementQty = function handleDecrementQty() {
  $('#table-body').on('click', '.decrement-quantity', function (e) {
    var productSku = $(e.currentTarget).parent().attr('id');
    var currentQty = $('#' + productSku + ' .quantity').text();
    var product = _.find(state.products, function (obj) {
      return obj.sku == productSku;
    });
